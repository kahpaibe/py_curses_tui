...
class ScrollableChoose(KeyCaptureDrawable):
                    ...
                    for yf in range(y, self._height + y):
                        Drawable.draw_str(
                            GenStr(
                                ...
                                if (
                                    yf < y + scrollbar_y
                                    or yf >= y + scrollbar_y + scrollbar_height
                                )
                                else self.scrollbar
                            ),
                            window,
    ...
    def key_behaviour(self, key: int) -> None:
        ...
        if key == curses.KEY_UP:
            if (self.capture_goto and self._cursor <= 0 and self._scroll <= 0) or (
                ...
                self._cursor = (self._cursor - 1) % min(
            ...
            if (
                ...
                try_self_call(self, self._choices[self._cursor + self._scroll].action)
    ...
    def _capture_take(self, origin: Tuple[int, int], direction: int) -> None:
        ...
        if direction == 0:  # from up to down
            ...
            if oy > maxy:
                ...
                self._cursor = (
                    oy - sy
                )  # set cursor to same y value, don't change scroll
